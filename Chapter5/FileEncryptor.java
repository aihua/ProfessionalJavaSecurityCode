 ...
 *	and a 128-bit Blowfish key. The key is then encrypted using
 *	a 1024-bit RSA key, which is kept in a JCE-based keystore.
 */
public class FileEncryptor {
  ...
  throws Exception {
    ...
    // Now create a new 128 bit blowfish key to encrypt the file itself.
    // This will be the session key.
    KeyGenerator blowfishKeyGenerator = KeyGenerator.getInstance("Blowfish");
    blowfishKeyGenerator.init(128);
    System.out.println("Generating session key...");
    Key blowfishKey = blowfishKeyGenerator.generateKey();
    System.out.println("Done generating key.");

    // Encrypt the blowfish key with the RSA cipher
    // and write it to the beginning of the file.
    byte[] encodedKeyBytes= rsaCipher.doFinal(blowfishKey.getEncoded());
    ...
    Cipher symmetricCipher = Cipher.getInstance("Blowfish/CBC/PKCS5Padding");
    symmetricCipher.init(Cipher.ENCRYPT_MODE, blowfishKey, spec);
  ...
  throws Exception {
    ...
    byte[] blowfishKeyBytes = rsaCipher.doFinal(encryptedKeyBytes);
    ...
    cipher.init(Cipher.DECRYPT_MODE, blowfishKey, spec);
